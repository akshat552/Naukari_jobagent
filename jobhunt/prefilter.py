...
def prefilter(jobs: list[Job], cfg: dict) -> list[Job]:
    ...
    max_applicants = cfg.get("max_applicants")
    cutoff = datetime.now(timezone.utc) - timedelta(days=max_age) if max_age else None

    kept, stats = [], {"title": 0, "location": 0, "age": 0, "applicants": 0}
    for j in jobs:
        title_and_desc = f"{j.title} {j.description[:400]}"
        if not _any_match(inc, j.title) and not _any_match(inc, title_and_desc):
            stats["title"] += 1
            continue
        if exc and _any_match(exc, j.title):
            stats["title"] += 1
            continue
        ...
        if max_applicants is not None and getattr(j, "applicants", None) is not None:
            if j.applicants > max_applicants:
                stats["applicants"] += 1
                continue
    ...
    print(f"  prefilter: {len(jobs)} -> {len(kept)} "
          f"(dropped title={stats['title']} location={stats['location']} stale={stats['age']} applicants={stats['applicants']})")
    return kept
