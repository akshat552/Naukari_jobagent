...
def _call_provider_with_fallback(provider: Provider, model: str, system: str, user: str,
                                 max_tokens: int, json_mode: bool = True) -> str:
    global _FAILOVER_ACTIVE
    or_key = (os.environ.get("OPENROUTER_API_KEY") or "").strip()

    # If already failed over in this session, use OpenRouter directly
    if _FAILOVER_ACTIVE and or_key and provider.name != "openrouter":
        from .providers import OpenRouterProvider
        or_provider = OpenRouterProvider()
        for fallback_model in ("openrouter/free", "nvidia/nemotron-3-super-120b-a12b:free", "minimax/minimax-m2.7:free"):
            try:
                time.sleep(1.0)
                return or_provider.complete(fallback_model, system, user, max_tokens, json_mode=json_mode)
            except Exception:
                continue
    ...
    try:
        ...
        if or_key and provider.name != "openrouter":
            _FAILOVER_ACTIVE = True
            print(f"  ! {provider.name} quota limit reached -> activating OpenRouter free fallback...")
            from .providers import OpenRouterProvider
            or_provider = OpenRouterProvider()
            for fallback_model in ("openrouter/free", "nvidia/nemotron-3-super-120b-a12b:free", "minimax/minimax-m2.7:free"):
                try:
                    ...
                    print(f"  ! openrouter model {fallback_model} retry: {or_err}")
                    continue
        raise e
...
def parse_json(raw: str) -> Any:
    ...
    for _, blob in sorted(candidates):
        try:
            ...
            cleaned_blob = re.sub(r',\s*([\]}])', r'\1', blob)
            try:
...
SCREEN_SYSTEM = """You evaluate tech job openings against a candidate profile for match quality.

Score each job 0-10:
  9-10  Direct bullseye on tech stack, seniority, domain, and architecture
  7-8   Solid match: core skills overlap well, slight differences in stack or scope
  5-6   Weak match: tangential stack, wrong seniority, or wrong focus
  0-4   Completely unqualified, completely irrelevant, or wrong discipline
...
def screen(jobs: list[Job], profile: dict, batch_size: int = 8, jd_chars: int = 1400,
    ...
    for start in range(0, len(jobs), batch_size):
        ...
        try:
            raw = _call_provider_with_fallback(
