...
class OpenRouterProvider(OpenAICompatProvider):
    """OpenRouter provider supporting free and multi-vendor models."""
    name = "openrouter"
    ...
    def complete(self, model: str, system: str, user: str, max_tokens: int,
        ...
        if json_mode:
            payload["response_format"] = {"type": "json_object"}
        headers = {
            "Authorization": f"Bearer {self._env(self.key_env)}",
            "HTTP-Referer": "https://github.com/jobhunt",
            "X-Title": "JobHunt Agent"
        }
        r = requests.post(
            f"{base}/chat/completions",
            headers=headers,
        ...
        try:
            ...
            raise LLMError(f"openrouter malformed reply: {r.text[:300]}") from e
...
DEFAULT_MODELS = {
    ...
    "openrouter": {"screen": "openrouter/free", "draft": "openrouter/free"},
