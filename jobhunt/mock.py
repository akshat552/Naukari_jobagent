...
from .fetch import parse_greenhouse, parse_lever, parse_ashby, parse_smartrecruiters, parse_workday, parse_linkedin, Job
        ...
        "content": [
            {
                ...
                "location": {"city": "Gurugram", "region": "Haryana", "country": "India"},
                "jobAd": {
                    "sections": {
                        "jobDescription": {"text": "<p>Build cloud-native C# .NET APIs and React web apps.</p>"}
                    }
                }
            }
        ]
...
WORKDAY = {
    "cognizant": {
        ...
        "jobPostings": [
            {
                "title": "Senior Software Engineer - .NET Core & Cloud",
                "externalPath": "/job/Bengaluru-India/Senior-Software-Engineer_JR999001",
                "locationsText": "Bengaluru, India",
                "postedOn": "2026-08-28",
                "bulletFields": ["JR999001", "Full time"]
            }
        ]
    }
}
...
LINKEDIN = {
    ...
    <ul>
        <li>
            <h3 class="base-search-card__title">Software Development Engineer (.NET / Azure)</h3>
            <h4 class="base-search-card__subtitle">Deloitte</h4>
            <span class="job-search-card__location">Hyderabad, Telangana, India</span>
            <a class="base-card__full-link" href="https://in.linkedin.com/jobs/view/sde-deloitte-12345678">Link</a>
            <time class="job-search-card__listdate" datetime="2026-08-28">2 days ago</time>
        </li>
    </ul>
    """
}
...
def fetch_all_mock(companies=None) -> list[Job]:
    ...
    for slug, body in WORKDAY.items():
        jobs += parse_workday(slug, slug.title(), body, base_host=f"{slug}.wd1.myworkdayjobs.com", site=f"{slug}_Careers")
    for slug, body in LINKEDIN.items():
        jobs += parse_linkedin(slug, slug.title(), body)
    total_boards = len(GREENHOUSE) + len(LEVER) + len(ASHBY) + len(SMARTRECRUITERS) + len(WORKDAY) + len(LINKEDIN)
    print(f"  [mock] {len(jobs)} postings from {total_boards} boards")
    return jobs
