...
from jobhunt.fetch import parse_ashby, parse_greenhouse, parse_lever, parse_smartrecruiters, parse_workday, parse_linkedin, strip_html
...
def test_smartrecruiters_maps_fields():
    jobs = parse_smartrecruiters("nagarro", "Nagarro", mock.SMARTRECRUITERS["nagarro"])
    assert len(jobs) == 1
    j = jobs[0]
    assert j.ats == "smartrecruiters"
    assert j.company == "Nagarro"
    assert j.title == "Full-Stack .NET Developer"
    assert "Gurugram" in j.location
    assert "7439999990001" in j.url
    assert "C# .NET" in j.description


def test_workday_maps_fields():
    jobs = parse_workday("cognizant", "Cognizant", mock.WORKDAY["cognizant"], base_host="cognizant.wd1.myworkdayjobs.com", site="Cognizant_Careers")
    assert len(jobs) == 1
    j = jobs[0]
    assert j.ats == "workday"
    assert j.company == "Cognizant"
    assert j.title == "Senior Software Engineer - .NET Core & Cloud"
    assert "Bengaluru" in j.location
    assert "JR999001" in j.url
    assert "JR999001" in j.description


def test_linkedin_maps_fields():
    jobs = parse_linkedin("deloitte", "Deloitte", mock.LINKEDIN["deloitte"])
    assert len(jobs) == 1
    j = jobs[0]
    assert j.ats == "linkedin"
    assert j.company == "Deloitte"
    assert j.title == "Software Development Engineer (.NET / Azure)"
    assert "Hyderabad" in j.location
    assert "sde-deloitte-12345678" in j.url
...
def test_job_ids_are_globally_unique_and_namespaced():
    ...
    assert all(re.match(r"^(greenhouse|lever|ashby|smartrecruiters|workday|linkedin):[^:]+:.+$", i) for i in ids)
...
def test_full_mock_funnel_keeps_only_the_five_real_matches():
    filters = dict(FILTERS, max_age_days=30)
    ...
    assert titles == [
        ...
        "Site Reliability Engineer",
        "Software Development Engineer (.NET / Azure)",
        "Software Development Engineer, Core Infra",
        "Software Engineer II, Distributed Systems",
...
def test_stale_posting_is_dropped_by_freshness_gate():
    kept = prefilter(fetch_all_mock(), dict(FILTERS, max_age_days=30))
...
def test_applicants_filter_drops_over_threshold():
    from jobhunt.fetch import Job
    j_under = Job(job_id="test:1", ats="test", company="A", title="Full-Stack .NET Developer",
                  location="India", url="http://example.com/1", description="desc", applicants=40)
    j_over = Job(job_id="test:2", ats="test", company="B", title="Full-Stack .NET Developer",
                 location="India", url="http://example.com/2", description="desc", applicants=150)
    kept = prefilter([j_under, j_over], dict(FILTERS, max_applicants=100))
    assert len(kept) == 1
    assert kept[0].job_id == "test:1"
